        ...
        <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
          {mostrarFavorito && (
            <button
              ...
              className={`shadow p-2.5 rounded-full transition cursor-pointer ${
                favorito
                  ? "text-[#33ea30] bg-green-100"
      ...
      <div className="px-6 pb-2">
        <div className="bg-white">
          ...
          <div className="flex items-center gap-0.5">
            <LocationIcon />
            <p className="text-xs">Calle 35 #676</p>
          </div>
            ...
            {empresa?.canchas
              ?.filter(
                (cancha, index, self) =>
                  index ===
                  self.findIndex(
                    (c) => c.tipo_cancha.tipo === cancha.tipo_cancha.tipo
                  )
              )
              .map((tipo, idx) => (
                <div
                  key={idx}
                  className="bg-blue-100 text-blue-900 text-xs px-2 py-1 rounded-lg font-semibold"
                >
                  {tipo.tipo_cancha.tipo}
                </div>
              ))}
          </div>
        </div>
        <div className="flex gap-4 justify-center my-2">
          {empresa?.servicios.map((servicio, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-center h-8 w-8 bg-green-200 p-1 rounded-xl justify-center"
              >
